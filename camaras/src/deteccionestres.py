...
# Dynamic confidence adjustment function
def dynamic_confidence(image, base_conf=0.6):
...
def process_frames():
    while True:
        ...
        conf = dynamic_confidence(frame)
        
        # Inferencia (usa source=frame para pasar imagen)
        results = model.predict(source=frame, imgsz=960, conf=conf, iou=0.4, max_det=100, verbose=False)
